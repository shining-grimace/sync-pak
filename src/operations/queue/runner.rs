use crate::{
    ...
    operations::queue::OperationQueue,
    operations::transfer::progress::NoopProgressObserver,
};
...
mod tests {
    ...
    use crate::{
        ...
        operations::execution::{ExecutionProgress, ExecutionState, OperationExecutor},
    ...
    impl OperationExecutor for Executor {
        ...
        ) -> Result<crate::operations::execution::ExecutionResult, CapabilityError> {
            self.completed
                .lock()
                .unwrap()
    ...
    fn runs_queued_operations_in_order_and_records_results() {
        let mut queue = crate::operations::queue::OperationQueue::default();
        queue.push(

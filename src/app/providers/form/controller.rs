...
use crate::{
    ...
    app::form_validation,
    app::onboarding::complete_welcome,
    app::providers::bucket_cache::{self as provider_bucket_cache, ProviderBucketCache},
    app::providers::form::credentials as provider_form_credentials,
    app::providers::form::state::{
    ...
    app::providers::list_controller as provider_list_controller,
...
pub(crate) fn configure(
    ...
    window.on_request_save_provider(move || {
        if let Some(window) = weak.upgrade() {
            crate::app::providers::form::secret_reveal::hide(&window);
...
fn verify(
    ...
    crate::app::providers::s3_verification_controller::start(
        weak.clone(),
        provider,
        credentials,
        diagnostics,

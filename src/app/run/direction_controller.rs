...
use crate::{
    AppWindow,
    app::connections::list_controller as connection_list_controller,
    app::diagnostics::{self as diagnostics_controller, SharedDiagnosticLog},
    app::run::direction_presentation::{archive_details, mode_label, remote_endpoint},
...
pub(crate) fn configure(
    ...
    window.on_cancel_run_direction(move || {
        if let Some(window) = weak.upgrade() {
            crate::app::preflight::controller::invalidate(&window);
        }
...
fn begin_preflight(
    ...
    match result {
        Ok(request) => {
            ...
            crate::app::preflight::controller::show_loading(&window);
...
fn start_preflight(
    ...
    crate::app::preflight::s3_controller::start(
        weak,
        request,
        configuration_path,
        diagnostics,

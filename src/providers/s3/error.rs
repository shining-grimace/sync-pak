...
use crate::providers::capabilities::{ProviderError, ProviderTransportError};

#[cfg(any(target_os = "android", test))]
use crate::providers::errors::endpoint_resolution::EndpointResolutionError;
#[cfg(any(target_os = "android", test))]
use crate::providers::errors::secure_connection::{
    SecureConnectionError, SecureConnectionErrorKind,
};
...
fn connector_error(error: &ConnectorError) -> ProviderError {
    ...
    if let Some(error) = find_source::<rustls::Error>(error) {
        return crate::platform::android::s3::tls_error::classify(error);
    }
    if error.is_user() {

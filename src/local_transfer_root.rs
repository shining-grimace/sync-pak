...
use crate::{
    ...
    local_inventory::{LocalInventoryAccess, LocalInventoryError, NativeLocalInventory},
};
...
impl LocalTransferRoot {
    ...
    pub fn verify(&self) -> io::Result<()> {
        match self {
            ...
            #[cfg(target_os = "android")]
            Self::AndroidTree(uri) => crate::android_document_tree::verify(uri),
        }
    }
    ...
    pub fn inventory(&self) -> Result<Inventory, LocalInventoryError> {
        match self {
            ...
            Self::AndroidTree(uri) => {
                crate::android_document_tree::inventory(uri).map_err(LocalInventoryError::Platform)
            }
        }
    }
    ...
    pub fn metadata(&self, relative: &RelativePath) -> io::Result<LocalEntryMetadata> {
        match self {
            ...
            #[cfg(target_os = "android")]
            Self::AndroidTree(uri) => {
                let metadata = crate::android_document_tree::metadata(uri, relative)?;
    ...
    pub fn open_read(&self, relative: &RelativePath) -> io::Result<fs::File> {
        match self {
            ...
            #[cfg(target_os = "android")]
            Self::AndroidTree(uri) => crate::android_document_tree::open_read(uri, relative),
        }
    }
    ...
    pub fn write(&self, relative: &RelativePath, contents: &[u8]) -> io::Result<()> {
        match self {
            ...
            #[cfg(target_os = "android")]
            Self::AndroidTree(uri) => {
                let mut file = crate::android_document_tree::open_write(uri, relative)?;
                file.write_all(contents)?;
                crate::android_document_file::finish_write(&mut file)
            }
        }
    }
    ...
    pub fn copy_from(&self, relative: &RelativePath, source: &Path) -> io::Result<()> {
        match self {
            ...
            Self::AndroidTree(uri) => {
                ...
                let mut destination = crate::android_document_tree::open_write(uri, relative)?;
                io::copy(&mut source, &mut destination)?;
                crate::android_document_file::finish_write(&mut destination)
            }
        }
    }
    ...
    pub fn create_directory_all(&self, relative: &RelativePath) -> io::Result<()> {
        match self {
            ...
            #[cfg(target_os = "android")]
            Self::AndroidTree(uri) => {
                crate::android_document_tree::create_directories(uri, relative)
            }
        }
    }
    ...
    pub fn delete(&self, relative: &RelativePath) -> io::Result<()> {
        match self {
            ...
            #[cfg(target_os = "android")]
            Self::AndroidTree(uri) => crate::android_document_tree::delete(uri, relative),
        }
    }
